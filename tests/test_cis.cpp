...
/**
 * This test verifies that `apex cis` operates under normal circumstances (no extreme p-values.)
 * We also include a set of covariates, which are associated with the outcome.
 */
TEST(CisTest, SimpleCisMappingWithCovariatesTest) {
  global_opts::reset();

  string output_prefix = "data/test_output.factor_test";
  ...
  vector<string> args = {"--bed", "data/factor_test.bed.gz", "--vcf", "data/factor_test.vcf.gz", "--field", "DS", "--cov", "data/factor_test.covariates_all.tsv", "--out", output_prefix, "--window", "1000000", "--long"};
  cis("tests", args.begin(), args.end());

  auto reader_long_table_truth = CisLongTableReader("data/factor_test.withcov.cis_long_table.txt.gz");

  // This is the file generated by the test case
  auto reader_long_table_test = CisLongTableReader("data/test_output.factor_test.cis_long_table.txt.gz");
